...
select emp_id, emp_name, dept_id, phone, email, salary, salary*0.2 as bonus from employee;
...
select emp_id as '사원번호', emp_name, gender, hire_date, dept_id, email, salary from employee where dept_id = 'sys';

-- EMP_NAME '사원명' 별칭 수정
select emp_id as '사원번호', emp_name as '사원명', gender, hire_date, dept_id, email, salary from employee where dept_id = 'sys';
...
select emp_id, emp_name, email, phone, email, salary, salary*02 as bonus from employee where retire_date is null;
...
select emp_id, emp_name, email, phone, salary, ifnull(salary, 0)
from employee
where dept_id = 'stg';
...
select emp_id, emp_name, eng_name, hire_date, ifnull(hire_date, curdate()) as RETIRE_DATE, salary
from employee
where dept_id = 'mkt';
/**************************************
...
select *
from employee
order by gender asc, salary asc;
...
select emp_id, emp_name, eng_name, hire_date, phone, salary as '급여'
from employee
where retire_date is null
order by 급여 asc;
...
-- 보너스가 1000 이상인 사원 조회
-- 보너스가 높은 사원 기준으로 정렬
select emp_id, emp_name, hire_date, email, phone, salary, ifnull(salary*0.2, 50) as bonus
from employee
...
select *
from employee
where emp_name = '일지매' and emp_name = '오삼식' and emp_name = '김삼순';
